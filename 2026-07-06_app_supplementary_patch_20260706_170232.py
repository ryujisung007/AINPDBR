...
## 4. 제약 조건
[근거 데이터] 기반 배합비 방향성(지방%·총고형분%·overrun 구체 수치, 조성×품질등급 범위 내)과
구조/공정 핵심 고려사항(코팅·리본·컨펙션·코어, 텁인 경우 tub_12l_extra_branches의 분산·마감 방식 포함),
저장수명/품질 리스크를 정리. 사용자가 입력한 원가/콜드체인/알러지 등 제약 메모를 그대로 반영하고,
워크스페이스의 R&D 마감(rdDeadline)을 명시. 근거로 사용한 챕터를 "참고: Ch.X"로 표기.

## 5. 검증 계획
사용자가 지정한 가상 소비자 표본 수·핵심 지표·배합비 테스트 라운드를 반영해 검증 설계를 구체화하고,
출시 시즌 컨텍스트(기온·이벤트)를 검증 조건에 명시. 지정되지 않은 항목은 합리적 기본값을 제안.

## 6. 기타 / 보충
사용자 메시지에 포함된 "[6번 기타/보충 전용 근거]" 항목을 그대로 우선 사용해 작성할 것
(예: 인접 조성/등급과의 비교, 대체 가능한 배합 옵션, 규격·라벨링 관련 챕터, 이 조합에서만 발생하는
부가 리스크 등). 그 항목이 비어 있으면 [근거 데이터] 전체에서 4.제약조건에 담지 못한 내용을
추가로 찾아보되, 없으면 반드시 "책에 명시되지 않음"으로 표시할 것 — 마케팅성 추측을 지어내지 말 것.
반드시 근거 챕터를 "참고: Ch.X"로 표기. 사용자가 입력한 기타 메모가 있으면 그 내용도 반영할 것."""
